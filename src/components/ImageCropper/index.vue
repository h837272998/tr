...
export default {
  ...
  methods: {
    ...
    upload() {
      ...
      ///////
      this.generateMd5(data2blob(createImgUrl, mime)).then(md5=>{
        fmData.append('md5',md5)
        fmData.append('access_token',_this.$store.getters.token)
        // 添加其他参数
        ...
        new Promise(function(resolve, reject) {
          ...
          function(resData) {
            if (_this.value) {
              _this.loading = 2;
              _this.$emit('crop-upload-success', resData, field, ki);
            }
          },
          // 上传失败
          function(sts) {
            if (_this.value) {
              ...
              _this.$emit('crop-upload-fail', sts, field, ki);
            }
          }
        );
