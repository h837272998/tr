<template>
  <div class="resource-container">
    <div class="search-container">
      ...
      <el-button slot="append" type="primary" icon="el-icon-search" circle @click="handleSearch" />
    </div>
    ...
    <div class="table-container">
      <div class="head-container">
        <el-button type="primary" style="margin: 10px 10px 0 20px" size="small" @click="handlePublice">发 帖</el-button>
        <el-pagination
...
<!--        <el-badge v-for="row in categoryList" style="margin: 0 20px 0 20px" type="warning" :value="row.number">-->
...
<!--        <el-divider />-->
      ...
      >
        ...
        >
          <template slot="header" slot-scope="{row}">
            ...
            >
              ...
              <el-link slot="reference">更多<i class="el-icon-caret-right" />&nbsp;</el-link>
            </el-popover>
            <el-popover
              trigger="click"
              placement="right"
            ...
            >
              <el-cascader
                ...
                clearable
              />
              <el-link slot="reference">信息<i class="el-icon-caret-right" />&nbsp;</el-link>
            </el-popover>
          </template>
          <template slot-scope="{row}">
            <div style="vertical-align: baseline">
              ...
              &nbsp;<el-tag v-for="row in row.categories" style="padding: 1px 1px 1px 1px;font-size: 8px" :type="generateTagType(row.name)" size="mini">{{ row.name }}</el-tag>
              <el-tag v-if="row.isLogged" style="padding: 1px 1px 1px 1px;font-size: 10px" title="登录可看" type="danger" size="mini"><i class="el-icon-lock" /></el-tag>
        ...
        >
          <template slot-scope="{row}">
            <span class="author">
              <el-link v-if="row.subordinates.collegeName" type="primary" style="font-size: 8px" @click="url.collegeId=row.subordinates.collegeId">{{ row.subordinates.collegeName }}</el-link>
              <el-link v-if="row.subordinates.majorName" type="primary" style="font-size: 8px" @click="url.majorId=row.subordinates.majorId">{{ row.subordinates.majorName }}</el-link>
              <el-link v-if="row.subordinates.courseName" type="primary" style="font-size: 8px" @click="url.courseId=row.subordinates.courseId">{{ row.subordinates.courseName }}</el-link>
            </span>
          </template>
        </el-table-column>
            ...
            <span class="author">
              ...
              >
                <el-link
                  ...
                  @click="row.popover=true"
                >{{ row.author }}</el-link>
                <user-dialog v-if="row.popover" :username="row.authorUsername" />
              </el-popover>
              <br>
            ...
            <span class="author">
              ...
              >
                <el-link
                  ...
                  type="primary"
                  style="line-height: 14px;font-size:12px"
                  @click="row.lastPopover=true"
                >{{ row.lastDiscussUsername }}</el-link>
                <user-dialog v-if="row.lastPopover" :username="row.lastDiscussUsername" />
              </el-popover><br>
        ...
        <div slot="append" style="text-align: center">
          ...
          <div v-loading="appendLoading" text="加载中..." />
...
export default {
  name: 'Communicate',
  components: { UserDialog, Pagination },
  filters: {
  ...
  methods: {
    ...
    handlePublice() {
      const routeUrl = this.$router.resolve({
        path: '/personal/operate/communicate'
      })
      window.open(routeUrl.href, '_blank')
    }
  }
